...
nsresult nsIFrame::PeekOffsetForParagraph(PeekOffsetStruct* aPos) {
  ...
  if (reachedLimit) {  // no "stop frame" found
    // Drill down to a leaf so scroll-into-view geometry, which only uses
    // text-frame offsets when the focus node is a Text node, gets a usable
    // position. Frame children include flattened-tree descendants, so this
    // also covers shadow-DOM content that the previous code reached via
