        ...
        let image_holder = unsafe {
            self.device_as_hal::<wgc::api::Vulkan, _, Option<VkImageHolder>>(
                ...
                |hal_device| {
                    ...
                    // Rationale:
                    //
                    // > In each element of pPlaneLayouts, the implementation
                    // > must ignore size. The implementation calculates the size
                    // > of each plane, which the application can query with
                    ...
                    //
                    // Since `wgpu_vkimage_create_with_dma_buf` above succeeded
                    // in creating the original DMABuf image, if we pass the
                    // same parameters, including the DRM format modifier and
                    // plane layouts, we can assume that this call will succeed
                    // too.
                    //
                    // The only thing we're adding is the `ALIAS` flag, because
                    // this aliases the original image.
