...
function createAttachmentMock(
  ...
  const pendingDownloadsWaiters = [];

  function notifyPendingDownload() {
    const ready = pendingDownloadsWaiters.filter(
      waiter => pendingDownloads.length >= waiter.count
    );
    for (const waiter of ready) {
      waiter.resolve();
    }
    for (const waiter of ready) {
      const index = pendingDownloadsWaiters.indexOf(waiter);
      if (index !== -1) {
        pendingDownloadsWaiters.splice(index, 1);
      }
    }
  }
  ...
  function waitForPendingDownloads(expectedCount) {
    ...
    return new Promise(resolve => {
      pendingDownloadsWaiters.push({ count: expectedCount, resolve });
    });
  }
