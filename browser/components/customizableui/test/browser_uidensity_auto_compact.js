...
add_task(async function test_collapsed_launcher_width_triggers_compact() {
  ...
  try {
    ...
    Assert.greater(
      wRatio,
      hRatio,
      "Window is narrow enough to isolate the launcher-width check"
    );
    ...
    // Expanding the launcher removes the collapsed condition, so compact should
    // disengage since the height check stays below the threshold.
