...
describe("settings ai features / Smart Window memories", () => {
  ...
  async function openAiFeaturePanel() {
    await TestUtils.waitForCondition(
      () => doc.getElementById("category-ai-features"),
      "Waiting for AI features category button"
    );
  ...
  it("shows correct empty state based on learning preference", async () => {
    await SpecialPowers.pushPrefEnv({
    ...
    await TestUtils.waitForCondition(() => {
      ...
      return (
        noMemoriesItem?.dataset.l10nId === "ai-window-no-memories-learning-off"
      );
    }, "Waiting for empty state to update to learning-off");
