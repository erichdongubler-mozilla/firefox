...
class AsyncGIconReader {
 public:
  ...
  RefPtr<GIconPromise> Start() {
    ...
    // Balanced with 'RefPtr<...> self' in Finish.
    AddRef();
    // We can't use g_file_load_bytes_async since it isn't available in some
    // versions of GLib we support.
 ...
 private:
  ...
  static void Finish(GObject* aIconFile, GAsyncResult* aResult, void* aData) {
    ...
    if (error) {
      // Note that in this case, content hasn't been set.
