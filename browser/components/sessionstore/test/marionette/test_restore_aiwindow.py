...
class TestAIWindowSessionRestore(SessionStoreTestCase):
    ...
    def test_window_mode_persists_across_restart(self):
        ...
        self.toggle_ai_window(False)
        self.assertFalse(
            self.is_ai_window(), msg="Window should be Classic after toggle back"
        )
        ...
        self.assertFalse(
            self.is_ai_window(),
            msg="Classic Window state should persist after restart",
        )

        self.assertEqual(
            self.get_tab_count(),
            tab_count,
            msg="Tab count should be preserved after second restart",
        )
