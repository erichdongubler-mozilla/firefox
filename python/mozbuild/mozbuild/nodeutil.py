...
def package_setup(
    ...
    try:
        ...
        if not skip_logging:
            print(f'Installing {package_name} for mach using "{" ".join(cmd)}"...')
        result = call_process(
...
def call_process(name, cmd, cwd=None, append_env={}):
    ...
    try:
        ...
        if cwd:
            print(f"\nError installing {name} in the {cwd} folder, aborting.")
        else:
            print(f"\nError installing {name}, aborting.")
