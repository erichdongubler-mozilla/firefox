...
pub(crate) struct TestGroupSplit<'a> {
    pub wildcard_entry: TestSplit<'a>,
...
impl<'a> TestGroupSplit<'a> {
    ...
    pub(crate) fn get_test_for_wpt<'b>(&'b mut self, test_name: &str) -> &'b TestSplit<'a> {
        let Self {
            wildcard_entry,
            specific_tests_by_name,
        } = self;
        let entry = specific_tests_by_name
            .get_mut(test_name)
            .unwrap_or(wildcard_entry);
...
impl TestSplit<'_> {
    pub fn none() -> Self {
        Self {
            divide_into: DivideInto::TestsInSameFile,
...
pub(crate) fn assert_seen<'a>(
    ...
    entries
        .flat_map(|(test_group, entry)| {
            std::iter::once(((test_group, None), &entry.wildcard_entry.seen)).chain(
                entry
                    .specific_tests_by_name
                    .iter()
                    .map(move |(test_name, entry)| ((test_group, Some(test_name)), &entry.seen)),
            )
        })
