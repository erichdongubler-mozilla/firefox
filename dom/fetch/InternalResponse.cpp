...
void InternalResponse::CloneBody(nsIInputStream** aStream, int64_t* aBodySize) {
  ...
  if (mWrappedResponse) {
    mWrappedResponse->CloneBody(aStream, aBodySize);
    return;
  }
  ...
  nsresult rv =
      NS_CloneInputStream(mBody, aStream, getter_AddRefs(replacementBody));
